...
function normalizeTheme(input){
  if(!input||typeof input!=='object')throw new Error('Tema non valido');
  const base=themePresets.windows2000,out={name:String(input.name||'Tema personale').slice(0,60)};
  for(const key of hexThemeKeys){const value=String(input[key]||base[key]);if(!/^#[0-9a-f]{6}$/i.test(value))throw new Error(`Colore ${key} non valido`);out[key]=value;}
  for(const [key,[min,max]] of Object.entries(themeNumbers)){const value=Number(input[key]??base[key]);if(!Number.isFinite(value))throw new Error(`Valore ${key} non valido`);out[key]=Math.max(min,Math.min(max,value));}
  out.iconOffsets=input.iconOffsets&&typeof input.iconOffsets==='object'?input.iconOffsets:{};
  return out;
}
function applyTheme(theme){
  const root=document.documentElement.style;
  root.setProperty('--desktop',theme.desktop);root.setProperty('--paper',theme.panel);root.setProperty('--panel',theme.panel);root.setProperty('--panel-bottom',`color-mix(in srgb, ${theme.panel}, #000 7%)`);root.setProperty('--title',theme.title);root.setProperty('--accent',theme.accent);root.setProperty('--ink',theme.text);root.setProperty('--muted',`color-mix(in srgb, ${theme.text}, ${theme.panel} 45%)`);root.setProperty('--radius',`${theme.radius}px`);root.setProperty('--border-size',`${theme.border}px`);root.setProperty('--spacing',`${theme.spacing}px`);root.setProperty('--depth',theme.shadow);root.setProperty('--highlight',theme.highlight/100);
}
function setupThemes(){
  const select=$('theme-select');
  select.innerHTML=Object.entries(themePresets).map(([id,t])=>`<option value="${id}">${t.name}</option>`).join('');
  const savedId=localStorage.getItem('pixel-sheet-theme-id');
  if(savedId&&themePresets[savedId])select.value=savedId;
  applyTheme(themePresets[select.value]||themePresets.windows2000);
  select.addEventListener('change',()=>{applyTheme(themePresets[select.value]);localStorage.setItem('pixel-sheet-theme-id',select.value);localStorage.removeItem('pixel-sheet-custom-theme');$('theme-status').textContent=`Tema applicato: ${themePresets[select.value].name}`;});
  $('theme-input').addEventListener('change',async event=>{try{const file=event.target.files?.[0];if(!file)return;const data=JSON.parse(await file.text());if(data.schemaVersion!==undefined&&Number(data.schemaVersion)!==1)throw new Error('Versione del tema non supportata');const custom=normalizeTheme(data);let option=select.querySelector('[value="custom"]');if(!option){option=document.createElement('option');option.value='custom';select.append(option);}option.textContent=`${custom.name} (personale)`;themePresets.custom=custom;select.value='custom';applyTheme(custom);localStorage.setItem('pixel-sheet-theme-id','custom');localStorage.setItem('pixel-sheet-custom-theme',JSON.stringify(custom));$('theme-status').textContent=`Tema importato: ${custom.name}`;}catch(error){$('theme-status').textContent=`Importazione non riuscita: ${error.message}`;}finally{event.target.value='';}});
  try{const custom=normalizeTheme(JSON.parse(localStorage.getItem('pixel-sheet-custom-theme')));themePresets.custom=custom;const option=document.createElement('option');option.value='custom';option.textContent=`${custom.name} (personale)`;select.append(option);if(savedId==='custom'){select.value='custom';applyTheme(custom);}}catch{}
}
